...
sap.ui.define([
	...
	function (NavigationHandler, Filter, JSONModel, FilterOperator, formatter, MessageBox, History) {
		...
		return sap.ui.controller("ui.s2p.mm.cntrl.ctrmass.update.HeaderDistribution.controller.HeaderDistribution", {
			formatter: formatter,
			...
			onInit: function () {
				...
				this.getView().byId("HdrDistTable").attachDataReceived(null,
					function (oEvt) {
						...
						if (dataCount <= 0) {
							that.getView().byId("EditHdrDist").setEnabled(false);

						} else {
							that.getView().byId("EditHdrDist").setEnabled(true);
...
/*				var p = sap.s4h.cfnd.featuretoggle.lib.featuresAsync();
			...
			HeaderfilterBarSearch: function (oEvent) {
				...
				var oComponent = this.getOwnerComponent();
				var oComponentModel = oComponent.getComponentModel();
				var oSearchField = this.getView().byId("HeaderSearchFld");
				...
				this.getView().byId("HdrDistTable").rebindTable();
			},

			handleFilters: function () {
				// the obtained itemkeys from the list report are parsed to form the filter object
				var oComponent = this.getOwnerComponent();
				var smrtDistTable = this.getView().byId("HdrDistTable");
				var oComponentModel = oComponent.getComponentModel();
			...
			onBeforeDocumentsTableRebind: function (oEvent) {
				...
				var mDistTable = this.getView().byId("HdrDistMTable");
				var smrtDistTable = this.getView().byId("HdrDistTable");
			...
			editHdrDist: function (oEvent) {
				...
				var oView = this.getView();
				this.oBundleText = oView.getModel("i18n").getResourceBundle();
				var oPGISmartRespTable = this.getView().byId("HdrDistTable");
				var mDistTable = this.getView().byId("HdrDistMTable");
				...
				this.getView().byId("EditHdrDistMtable").setEnabled(false);
			...
			onChange: function (oEvent) {
				...
				if (regExpCom.test(checkVal) || regExpDot.test(checkVal)) {
					...
					if (inputVal !== "" && parseFloat(inputVal) <= 100) {
						...
						var changeString = "/C_CntrlPurContrHdrDistr(CentralPurchaseContract='" + changedContract + "',DistributionKey='" +
							changedDistribution + "')";
						var oTable = this.getView().byId("HdrDistMTable");
						...
						if (!this.getView().byId("ResetBtn").getEnabled()) {
							this.getView().byId("HdrDistMTable").getModel().resetChanges();
						}
					} else {
						this.getView().byId("ApplyBtn").setEnabled(false);
						this.getView().byId("SimulateBtn").setEnabled(false);
				...
				var tablelength = this.getView().byId("HdrDistMTable").getBindingInfo("rows").binding.oList.length;
				for (var tabindex = 0; tabindex < tablelength; tabindex++) {
					if (this.getView().byId("HdrDistMTable").getBindingInfo("rows").binding.oList[tabindex].GroupHeaderReference !== 1 && this.getView()
						.byId("HdrDistMTable").getBindingInfo("rows").binding.oList[tabindex].ReferenceHeaderDistributionKey !== -1) {
						if (this.getView().byId("HdrDistMTable").getBindingInfo("rows").binding.oList[tabindex].CntrlPurContrDistributionPct > 100 ||
							this.getView().byId("HdrDistMTable").getBindingInfo("rows").binding.oList[tabindex].CntrlPurContrDistributionPct === "") {
							this.getView().byId("ApplyBtn").setEnabled(false);
							this.getView().byId("SimulateBtn").setEnabled(false);
							break;
						}
			...
			onReset: function (oEvent) {
				...
				oView.byId("HdrDistMTable").getModel().resetChanges();
				oView.byId("ApplyBtn").setEnabled(false);
				oView.byId("SimulateBtn").setEnabled(false);
				oView.byId("ResetBtn").setEnabled(false);
				var oTable = oView.byId("HdrDistMTable");
			...
			ApplyChange: function (oEvent) {
				...
				if (!oDialog) {
					oDialog = sap.ui.xmlfragment(oView.getId(),
						"ui.s2p.mm.cntrl.ctrmass.update.HeaderDistribution.fragment.HdrDistrApplyChangesDialog", this);
			...
			onJobDescriptionChange: function (oEvent) {
				...
				if (oEvent.getParameter("value") === "") {
					this.getView().byId("CanvasHdrupdateJobConfirmationBtn").setEnabled(false);
					...
					if (this.validationErrFields.length < 1) {
						this.getView().byId("CanvasHdrupdateJobConfirmationBtn").setEnabled(true);
					}
			...
			onMetadataLoadedOfApplyChangesDialog: function (oSetNewValuesDialog) {
				...
				var oModelContext;
				if (oSetNewValuesDialog && oSetNewValuesDialog.getId().indexOf("HdrDistApplyChangesDialog") > -1) {
			...
			validateReasonCode: function (oEvent) {
				...
				var valueFromComboBox = this.getView().byId("CanvasHdrPurchasingDocVersionReasonCode-comboBoxEdit").getValue();
				if (newValue === "" && valueFromComboBox !== "") {
					this.getView().byId("CanvasHdrPurchasingDocVersionReasonCode").setValueState("Error");
					...
					this.getView().byId("CanvasHdrPurchasingDocVersionReasonCode").setValueState("None");
			...
			onPressCancelButton: function () {
				...
				var oComponent = this.getOwnerComponent();
				var oSetNewValuesModel = oComponent.getModel();
				var aContext = this.getView().byId("HdrDistApplyChangesDialog").getContent()[0].getBindingContext();
				oSetNewValuesModel.deleteCreatedEntry(aContext);
				//close and destroy the dialog
				this.getView().byId("HdrDistApplyChangesDialog").close();
				this.getView().byId("HdrDistApplyChangesDialog").destroy();
			},
			...
			onSubmitChnges: function (oEvent) {
				...
				var ReasonCodeValue = this.getView().byId("CanvasHdrPurchasingDocVersionReasonCode").getValue();
				//fetching the value for Purchaser Note from the apply mass changes dialog
				var PurchaserNoteValue = this.getView().byId("CanvasHdrPurgDocNoteText").getValue();
				//fetching the value for Job Description from the apply mass changes dialog
				var sComment = this.getView().byId("HdrDistApplyChangesDialogmassChangeJobCommentBox").getValue();
				this.getView().byId("HdrDistMTable").getModel().resetChanges();
				...
				for (var i = 0; i < this.newData.length; i++) {
					if (this.newData[i].hasOwnProperty("__metadata") === true) {
						var chngdCtrDist = this.newData[i].CentralPurchaseContract + "*" + this.newData[i].DistributionKey;
				...
				for (i = 0; i < this.updateContracts.length; i++) {
					...
					var ctrIndex = url.search("/C_CntrlPurContrHdrDistr");
						...
						if (jobId !== undefined && jobId !== null) {
							...
							var dialog = new sap.m.Dialog({
								...
								endButton: new sap.m.Button({
									...
									press: function () {
										dialog.close();
										dialog.getParent().byId("HdrDistMTable").setVisible(false);
										dialog.getParent().byId("HdrDistTable").setVisible(true);
										...
										dialog.getParent().byId("HdrDistTable").getModel().refresh();
									}
								}),
								afterClose: function () {
									dialog.destroy();
							...
							that.getView().byId("HdrDistMTable").setVisible(false);
							that.getView().byId("HdrDistTable").setVisible(true);
							...
							that.getView().byId("HdrDistTable").getModel().refresh();
