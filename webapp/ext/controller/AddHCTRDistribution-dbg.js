...
/*			var p = sap.s4h.cfnd.featuretoggle.lib.featuresAsync();
		...
		onPressAddItmDist: function () {
			...
			var mAddItmDistForm;
			// if(tab2count)
			mAddItmDistForm = oView.byId("AddHCTRItemDistribution");
			if (!mAddItmDistForm) {
				mAddItmDistForm = sap.ui.xmlfragment(oView.getId(), "ui.s2p.mm.cntrl.ctrmass.update.ext.fragment.AddHCTRItemDistribution", this);
			...
			this.byId("ProcmtHubCompanyCodeGroupHierGroupElement").setLabel("Company Code Group");
			var enableSimulationFeature = this.AddHCTRDistribution.enableSimulationFeature.bind(this);
			...
			this.fieldLength = oView.getModel().getMetaModel().getODataEntityType(
				"MM_PUR_CNTRLCTRMASS_UPDT_SRV.C_CPurConHierItmDistrMassUpdtType");
		...
		onPressAddHdrDist: function () {
			var oView = this.getView();
			var mAddHdrDistForm;
			mAddHdrDistForm = oView.byId("AddHCTRHeaderDistribution");
			if (!mAddHdrDistForm) {
				mAddHdrDistForm = sap.ui.xmlfragment(oView.getId(), "ui.s2p.mm.cntrl.ctrmass.update.ext.fragment.AddHCTRHeaderDistribution", this);
			...
			//this is object is further used to derive the maxlength of the editable fields.
			this.fieldLength = oView.getModel().getMetaModel().getODataEntityType(
				"MM_PUR_CNTRLCTRMASS_UPDT_SRV.C_CPurConHierHdrDistrMassUpdtType");
			//code for showing the count
			this.setAffectedDocumentsCount();
		...
		validateHCTRPlant: function (oEvent) {
			...
			if (newValue.length <= Number(maxLength) && newValue !== undefined && newValue !== "") {
				...
				if (validated) {
					// that.AddHCTRDistribution.enableApplyChange("Plant", that);
					// that.AddHCTRDistribution.validateStorageLocation();
					enableApplyChange = that.AddHCTRDistribution.enableApplyChange.bind(that);
					enableApplyChange("Plant");
					validateStorageLocation = that.AddHCTRDistribution.validateStorageLocation.bind(that);
							...
							if (oResponse && oResponse.results) {
								if (oResponse.results.length === 0) {
									...
									if (that.getView().byId("Plant").getMandatory() === true) {
										var dependendsofMandateFields = that.AddHCTRDistribution.dependendsofMandateFields.bind(that);
		...
		validateHCTRPurchasingOrg: function (oEvent) {
			//maxlength for the plant field extracted from metadata.
			...
			if (newValue.length <= Number(maxLength) && newValue !== undefined && newValue !== "") {
				//since the value was selectedc from value help no validation check is done and directly call method enableApplyChange method for further processing
				if (validated) {
					//	that.AddHCTRDistribution.enableApplyChange("ProcmtHubPurchasingOrg", that);
					var enableApplyChange = that.AddHCTRDistribution.enableApplyChange.bind(that);
		...
		validateHCTRPurchasingGrp: function (oEvent) {
			...
			if (newValue.length <= Number(maxLength) && newValue !== undefined && newValue !== "") {
				if (validated) {
					//since the value was selectedc from value help no validation check is done and directly call method enableApplyChange method for further processing
					//that.AddHCTRDistribution.enableApplyChange("ProcmtHubPurchasingGroup", that);
					enableApplyChange = that.AddHCTRDistribution.enableApplyChange.bind(that);
		...
		validateDependentFieldsOfHCTR: function () {
			var PlantVal, PurchasingOrg, StorageLocation, PurchasingGrp, PurchasingDocumentType;
			PlantVal = this.getView().byId("Plant").getValue();
			if (PlantVal) {
				var PlantValidate = this.AddHCTRDistribution.validatePlant.bind(this);
				PlantValidate();
				//	this.AddHCTRDistribution.validatePlant();
			}
			PurchasingOrg = this.getView().byId("ProcmtHubPurchasingOrg").getValue();
			if (PurchasingOrg) {
				//	this.AddHCTRDistribution.validatePurchasingOrg();
				var validatePurchasingOrg = this.AddHCTRDistribution.validatePurchasingOrg.bind(this);
				validatePurchasingOrg();
			}
			PurchasingGrp = this.getView().byId("ProcmtHubPurchasingGroup").getValue();
			if (PurchasingGrp) {
				var validatePurchasingGrp = this.AddHCTRDistribution.validatePurchasingGrp.bind(this);
				validatePurchasingGrp();
				//	this.AddHCTRDistribution.validatePurchasingGrp();
			}
			StorageLocation = this.getView().byId("StorageLocation").getValue();
			if (StorageLocation) {
				//	this.AddHCTRDistribution.validateStorageLocation();
				var validateStorageLocation = this.AddHCTRDistribution.validateStorageLocation.bind(this);
				validateStorageLocation();
			}
			PurchasingDocumentType = this.getView().byId("PurchasingDocumentType").getValue();
			if (PurchasingDocumentType) {
				var validatePurchasingDocumentType = this.AddHCTRDistribution.validatePurchasingDocumentType.bind(this);
				validatePurchasingDocumentType();
				//that.AddHCTRDistribution.validatePurchasingDocumentType();
			}
		},

		hideDependentfieldsOfHCTR: function () {
			var spliceIndex;
			...
			this.getView().byId("PurchasingDocumentType").setEditable(false);
			this.getView().byId("PurchasingDocumentType").setValue("");
			spliceIndex = this.MandateFieldChanged.indexOf("PurchasingDocumentType");
			if (spliceIndex > -1) {
				this.MandateFieldChanged.splice(spliceIndex, 1);
			}
		},

		//this method is triggered when the value is entered in Company Code field and on tab out for HCTR
		validateHCTRCompanyCode: function (oEvent) {
			//maxlength for the plant field extracted from metadata.
			var maxLength = this.getView().getModel().getMetaModel().getODataProperty(this.fieldLength,
				"ProcmtHubCompanyCode").maxLength;
			var that = this;
			var ValueStateIndex, enableApplyChange, validateDependentFieldsOfHCTR;
			// oEvent.getSource().getBinding("value").attachEventOnce(
			...
			if (newValue.length <= Number(maxLength) && newValue !== undefined && newValue !== "") {
				...
				if (validated) {
					...
					if (that.getView().byId("ProcmtHubPurgDocItmCategory").getValue() === "L") {
						that.getView().byId("PurchasingDocumentType").setEditable(true);
					}
					that.getView().byId("StorageLocation").setEditable(true);
					enableApplyChange = that.AddHCTRDistribution.enableApplyChange.bind(that);
					enableApplyChange("ProcmtHubCompanyCode");
					//	that.AddHCTRDistribution.enableApplyChange("ProcmtHubCompanyCode", that);
					...
					//that.AddHCTRDistribution.validateDependentFieldsOfHCTR(that);
					validateDependentFieldsOfHCTR = that.AddHCTRDistribution.validateDependentFieldsOfHCTR.bind(that);
					validateDependentFieldsOfHCTR();
				} else {
					var aFilters = [];
					var oModel = this.getView().getModel();
					var url = "/C_ProcmtHubGroupCompanyCodeVH";
					var oUoMFilter = new sap.ui.model.Filter({
						path: "ProcmtHubCompanyCode",
						operator: sap.ui.model.FilterOperator.EQ,
						value1: newValue
						...
						success: function (oResponse) {
							...
							if (oResponse && oResponse.results) {
								if (oResponse.results.length === 0) {
									...
									that.ProcmtHubCompanyCodeGroupingID = "";
									that.getView().byId("ProcmtHubCompanyCodeGroupingID").setValue(that.ProcmtHubCompanyCodeGroupingID);
									var hideDependentfieldsOfHCTR = that.AddHCTRDistribution.hideDependentfieldsOfHCTR.bind(that);
									hideDependentfieldsOfHCTR();
									//	that.AddHCTRDistribution.hideDependentfieldsOfHCTR();
									that.getView().byId("DialogApplyBtn").setEnabled(false);
									that.getView().byId("DialogSimulateBtn").setEnabled(false);
									var dependendsofMandateFields = that.AddHCTRDistribution.dependendsofMandateFields.bind(that);
									dependendsofMandateFields();
									...
									that.ProcmtHubCompanyCodeGroupingID = oResponse.results[0].ProcmtHubCompanyCodeGroupingID;
									that.getView().byId("ProcmtHubCompanyCodeGroupingID").setValue(that.ProcmtHubCompanyCodeGroupingID);
		...
		//This method is triggered when the value is entered in document category field and on  tab out For HCTR.
		validateHCTRItmCategory: function (oEvent) {
			var ValueStateIndex;
			var newValue = oEvent.getParameter("newValue");
			// if (newValue) {
			this.getView().byId("DialogResetBtn").setEnabled(true);
			...
			// 	this.AddHCTRDistribution.viewControl.getView().byId("DialogResetBtn").setEnabled(true);
			// }
			...
			if (newValue === "L" && this.getView().byId("ProcmtHubCompanyCode").getValue()) {
				this.getView().byId("PurchasingDocumentType").setEditable(true);
			} else {
				this.getView().byId("PurchasingDocumentType").setEditable(false);
				var OutlineagreementType = this.getView().byId("PurchasingDocumentType").getValue();
				if (OutlineagreementType) {
					this.getView().byId("PurchasingDocumentType").setValue("");
				}
				var smartIndex = this.MandateFieldChanged.indexOf("PurchasingDocumentType");
				if (smartIndex > -1) {
			...
			if (newValue === "L" || newValue === "K") {
				...
				var OutlineAgreementType = this.getView().byId("PurchasingDocumentType").getValue();
				smartFieldIndex = this.MandateFieldChanged.indexOf("PurchasingDocumentType");
				if (category === "K" && smartFieldIndex > -1 && OutlineAgreementType) {
					this.MandateFieldChanged.splice(smartFieldIndex, 1);
					this.getView().byId("PurchasingDocumentType").setValue("");
				}
				//	this.AddHCTRDistribution.enableApplyChange("ProcmtHubPurgDocItmCategory", this);
				var enableApplyChange = this.AddHCTRDistribution.enableApplyChange.bind(this);
				...
				var dependendsofMandateFields = this.AddHCTRDistribution.dependendsofMandateFields.bind(this);
				dependendsofMandateFields();
		...
		//this.AddHCTRDistribution.viewControl method is triggered when the value is entered in Distribution Percenatge field and on tab out.
		validateHCTRDistributionPercentage: function (oEvent) {
			var ValueStateIndex;
			this.getView().byId("DialogResetBtn").setEnabled(true);
			var newValue = oEvent.getParameter("newValue");
			...
			if (newValue !== "" && parseFloat(newValue) <= 100 && parseFloat(newValue) >= 0 && ValueState !== sap.ui.core.ValueState.Error) {
				//	this.AddHCTRDistribution.enableApplyChange("CntrlPurContrDistributionPct", this);
				var enableApplyChange = this.AddHCTRDistribution.enableApplyChange.bind(this);
		...
		validateHCTRStorageLocation: function (oEvent) {
			//maxlength for the plant field extracted from metadata.
			...
			if (newValue.length <= Number(maxLength) && newValue !== undefined && newValue !== "") {
				...
				if (validated) {
					...
					//	that.AddHCTRDistribution.enableApplyChange("StorageLocation", that);
					enableApplyChange = that.AddHCTRDistribution.enableApplyChange.bind(that);
				...
				//check for enabling the Apply Mass Changes button 
				if ((that.MandateFieldChanged.length === 5 && that.DocumentCategory === "K" && that.ValueStateErrorFields.length === 0) || (
						that.MandateFieldChanged
						.length === 7 && that.DocumentCategory ===
						"L" && that.ValueStateErrorFields.length === 0)) {
					that.getView().byId("DialogApplyBtn").setEnabled(true);
					that.getView().byId("DialogSimulateBtn").setEnabled(true);
		...
		//This method is triggered when the value is entered in plant field and on tab out.
		validateHCTRPurchasingInfoRecordUpdateCode: function (oEvent) {
			//maxlength for the plant field extracted from metadata.
			...
			if (newValue.length <= Number(maxLength) && newValue !== undefined && newValue !== "") {
				...
				if (validated) {
					//since the value was selectedc from value help no validation check is done and directly call method enableApplyChange method for further processing
					//	that.AddHCTRDistribution.enableApplyChange("PurchasingInfoRecordUpdateCode", that);
					enableApplyChange = that.AddHCTRDistribution.enableApplyChange.bind(that);
		...
		validateHCTRPurchasingDocVersionReasonCode: function (oEvent) {
		...
		validateHCTRPaymentTerms: function (oEvent) {
			...
			if (newValue.length <= Number(maxLength) && newValue !== undefined && newValue !== "") {
				...
				if (validated) {
					...
					//	that.AddHCTRDistribution.enableApplyChange("PaymentTerms", that);
				}
				//Validation check for the values directly entered by user and not from value help
				else {
					var aFilters = [];
		...
		validateHCTRPurchasingDocumentType: function (oEvent) {
			var maxLength = this.getView().getModel().getMetaModel().getODataProperty(this.fieldLength,
				"PurchasingDocumentType").maxLength;
			this.companyCodeChanged = "";
			...
			if (oEvent) {
				...
				newValue = this.getView().byId("PurchasingDocumentType").getValue();
				//flag set when company code triggered this method
				this.companyCodeChanged = "X";
			}
			...
			if (newValue.length <= Number(maxLength) && newValue !== undefined && newValue !== "") {
				...
				if (validated) {
					enableApplyChange = that.AddHCTRDistribution.enableApplyChange.bind(that);
					enableApplyChange("PurchasingDocumentType");
					//	that.AddHCTRDistribution.enableApplyChange("PurchasingDocumentType", that);
				}
				//Validation check for the values directly entered by user and not from value help
				else {
					var aFilters = [];
						...
						success: function (oResponse) {
							...
							if (oResponse && oResponse.results) {
								if (oResponse.results.length === 0) {
									...
									// that.getView().byId("PurgDocNoteText").setEnabled(false);
									// that.getView().byId("PurchasingDocVersionReasonCode").setEnabled(false);
									var mandateIndex = that.MandateFieldChanged.indexOf("PurchasingDocumentType");
									if (mandateIndex > -1) {
										that.MandateFieldChanged.splice(mandateIndex, 1);
									}
									that.getView().byId("PurchasingDocumentType").setValueState("Error");
									ValueStateIndex = that.ValueStateErrorFields.indexOf("PurchasingDocumentType");
									if (ValueStateIndex < 0) {
										that.ValueStateErrorFields.push("PurchasingDocumentType");
									}
									...
									that.getView().byId("PurchasingDocumentType").setValueStateText(errMsg);
								} else {
									//if the value entered is a valid value then enableApplyChange is triggered for further processing 
									that.getView().byId("PurchasingDocumentType").setValueState("None");
									ValueStateIndex = that.ValueStateErrorFields.indexOf("PurchasingDocumentType");
		...
		onReset: function (oEvent) {
			...
			if (oSource ===
				...
				this.getView().byId("smartFormHCTRItemDistribution").getModel().resetChanges();
				resetSamrtFormMetadat = this.AddHCTRDistribution.resetSamrtFormMetadat.bind(this);
				resetSamrtFormMetadat("smartFormHCTRItemDistribution");
				//this.AddHCTRDistribution.resetSamrtFormMetadat(this, "smartFormItemDistribution");
				...
				// 	this.getView().byId("DialogItemApplyBtn").setEnabled(false);
				// this.getView().byId("DialogItemResetBtn").setEnabled(false);
				this.getView().byId("smartFormHCTRHeaderDistribution").getModel().resetChanges();
				resetSamrtFormMetadat = this.AddHCTRDistribution.resetSamrtFormMetadat.bind(this);
				resetSamrtFormMetadat("smartFormHCTRHeaderDistribution");
				//this.AddHCTRDistribution.resetSamrtFormMetadat(this, "smartFormItemDistribution");
				//this.getView().byId("ItemCntrlPurContrDistributionPct").setValue(parseFloat(Number(0)).toFixed(3));
			}
		...
		resetSamrtFormMetadat: function (smartFormId) {
			...
			for (var outerIndex = 0; outerIndex < 2; outerIndex++) {
				for (var innerIndex = 0; innerIndex < 5; innerIndex++) {
					...
					if (fieldname === "PurchasingDocumentType" || fieldname === "ProcmtHubPurchasingGroup" || fieldname ===
		...
		enableApplyChange: function (fieldId) {
			...
			if ((this.MandateFieldChanged.length === 5 && this.DocumentCategory === "K" && this.ValueStateErrorFields.length === 0) ||
				(this.MandateFieldChanged.length === 7 && this.DocumentCategory === "L" && this.ValueStateErrorFields.length === 0)) {
		...
		onCancelDialog: function (oEvent) {
			...
			if (oSource ===
				...
				dialogId = "AddHCTRHeaderDistribution";
			} else if (oSource ===
				"ui.s2p.mm.cntrl.ctrmass.update::sap.suite.ui.generic.template.ListReport.view.ListReport::C_PurCntrlContrItmMassUpdt--AddHCTRItemDistribution"
			) {
				dialogId = "AddHCTRItemDistribution";
			}
			if (dialogId) {
		...
		validateHCTRPurchaserNote: function (oEvent) {
