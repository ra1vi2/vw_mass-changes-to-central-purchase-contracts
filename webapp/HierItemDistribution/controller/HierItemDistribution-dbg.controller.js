...
sap.ui.define([
	...
	function (NavigationHandler, Filter, JSONModel, FilterOperator, formatter, MessageBox, History) {
		...
		return sap.ui.controller("ui.s2p.mm.cntrl.ctrmass.update.HierItemDistribution.controller.HierItemDistribution", {
			formatter: formatter,
			...
			onInit: function () {
				...
				this.getView().byId("HierItmDistTable").attachDataReceived(null,
					function (oEvt) {
						...
						if (dataCount <= 0) {
							that.getView().byId("EditHierItmDist").setEnabled(false);
						} else {
							that.getView().byId("EditHierItmDist").setEnabled(true);
						}
					});
			},
...
/*				var p = sap.s4h.cfnd.featuretoggle.lib.featuresAsync();
			...
			onBeforeItemTableRebind: function (oEvent) {
				...
				var mDistTable = this.getView().byId("HierItmDistMTable");
				var smrtDistTable = this.getView().byId("HierItmDistTable");
				this.aPGISorters = [];
				...
				if (!oSorter) {
					...
					oBindingParams.sorter[3] = new sap.ui.model.Sorter("PurgDocItemDistributionKey", false, false);
				}
			...
			editHierItmDist: function (oEvent) {
				//the obtained data from the read call is converted to json model 
				var dataArray = [];
				if (this.getView().getModel("HierItemDistModel") !== undefined) {
					this.getView().getModel("HierItemDistModel").setData({
						modelData: dataArray
					});
				}
				var that = this;
				var oView = this.getView();
				this.oBundleText = oView.getModel("i18n").getResourceBundle();
				var oPGISmartRespTable = this.getView().byId("HierItmDistTable");
				var mDistTable = this.getView().byId("HierItmDistMTable");
				var Header = that.oBundleText.getText("HierItemDistribution") + " (" + oPGISmartRespTable._getRowCount() + ")";
				...
				this.getView().byId("EditHierItmDistMtable").setEnabled(false);
				...
				var sUrl = "/C_CPurConHierItmDistrMassUpdt";
				...
				odataModel.read(sUrl, {
					...
					sorters: [new sap.ui.model.Sorter("FormattedPurchaseContractItem", false, false), new sap.ui.model.Sorter(
						"ReferenceHeaderDistributionKey", true, false), new sap.ui.model.Sorter("PurgDocItemDistributionKey", false, false)],
					success: function (oData) {
						...
						that.getView().setModel(oItemDistModel, "HierItemDistModel");
						...
						var oSorterDkey = new sap.ui.model.Sorter("PurgDocItemDistributionKey", false, false);
			...
			onChange: function (oEvent) {
				...
				if (regExpCom.test(checkVal) || regExpDot.test(checkVal)) {
					...
					if (inputVal !== "" && parseFloat(inputVal) <= 100) {
						...
						var oTable = this.getView().byId("HierItmDistMTable");
						...
						var changeString = "/C_CPurConHierItmDistrMassUpdt(CentralPurchaseContract='" + Contract + "',CentralPurchaseContractItem='" +
							contractItem + "',PurgDocItemDistributionKey='" + changedDistribution +
							"')";
						oTable = this.getView().byId("HierItmDistMTable");
						...
						if (!this.getView().byId("ResetBtn").getEnabled()) {
							this.getView().byId("HierItmDistMTable").getModel().resetChanges();
						}
				...
				var tablelength = this.getView().byId("HierItmDistMTable").getBindingInfo("rows").binding.oList.length;
				for (var tabindex = 0; tabindex < tablelength; tabindex++) {
					if (this.getView().byId("HierItmDistMTable").getBindingInfo("rows").binding.oList[tabindex].GroupHeaderReference !== 1 && this.getView()
						.byId("HierItmDistMTable").getBindingInfo("rows").binding.oList[tabindex].ReferenceHeaderDistributionKey !== -1) {
						if (this.getView().byId("HierItmDistMTable").getBindingInfo("rows").binding.oList[tabindex].CntrlPurContrDistributionPct > 100 ||
							this.getView().byId("HierItmDistMTable").getBindingInfo("rows").binding.oList[tabindex].CntrlPurContrDistributionPct === "") {
							this.getView().byId("ApplyBtn").setEnabled(false);
							this.getView().byId("SimulateBtn").setEnabled(false);
							break;
						}
			...
			onReset: function (oEvent) {
				...
				oTable.getModel("HierItemDistModel").refresh(true);
			...
			ApplyChange: function (oEvent) {
				...
				if (!oDialog) {
					oDialog = sap.ui.xmlfragment(oView.getId(),
						"ui.s2p.mm.cntrl.ctrmass.update.HierItemDistribution.fragment.HierItemDistrApplyChangesDialog", this);
			...
			onMetadataLoadedOfApplyChangesDialog: function (oSetNewValuesDialog) {
				...
				var oModelContext;
				if (oSetNewValuesDialog && oSetNewValuesDialog.getId().indexOf("HierItmDistApplyChangesDialog") > -1) {
			...
			onPressCancelButton: function () {
				...
				var oComponent = this.getOwnerComponent();
				var oSetNewValuesModel = oComponent.getModel();
				var aContext = this.getView().byId("HierItmDistApplyChangesDialog").getContent()[0].getBindingContext();
				oSetNewValuesModel.deleteCreatedEntry(aContext);
				this.getView().byId("HierItmDistApplyChangesDialog").close();
				this.getView().byId("HierItmDistApplyChangesDialog").destroy();
			},
			...
			onSubmitChnges: function (oEvent) {
				...
				var sComment = this.getView().byId("HierItmDistApplyChangesDialogmassChangeJobCommentBox").getValue();
				this.getView().byId("HierItmDistMTable").getModel().resetChanges();
				...
				for (var i = 0; i < this.newData.length; i++) {
					if (this.newData[i].hasOwnProperty("__metadata") === true) {
						var chngdCtrDist = this.newData[i].FormattedPurchaseContractItem + "*" + this.newData[i].PurgDocItemDistributionKey;
				...
				for (i = 0; i < this.updateContracts.length; i++) {
					...
					var ctrIndex = url.search("/C_CPurConHierItmDistrMassUpdt");
						...
						if (jobId !== undefined && jobId !== null) {
							...
							var dialog = new sap.m.Dialog({
								...
								endButton: new sap.m.Button({
									...
									press: function () {
										dialog.close();
										dialog.getParent().byId("HierItmDistMTable").setVisible(false);
										dialog.getParent().byId("HierItmDistTable").setVisible(true);
										...
										dialog.getParent().byId("HierItmDistTable").getModel().refresh();
									}
								}),
								afterClose: function () {
									dialog.destroy();
							...
							that.getView().byId("HierItmDistMTable").setVisible(false);
							that.getView().byId("HierItmDistTable").setVisible(true);
							...
							that.getView().byId("HierItmDistTable").getModel().refresh();
